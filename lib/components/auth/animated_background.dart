...
class _AnimatedBackgroundState extends State<AnimatedBackground> with TickerProviderStateMixin {
  ...
  void _handleBurstAnimation() {
      ...
      setState(() {
         if (val < 0.4) { // Shake
           ...
           _shakeIntensity = 0.0;
           _opacity = 1.0;
           double elastic =  pow(2, -10 * progress) * sin((progress * 10 - 0.75) * (2 * pi) / 3) + 1;
  ...
  ShapeType _targetShape() {
    switch (widget.stepIndex % 4) {
        ...
        case 2: return ShapeType.triangle;
        case 3: return ShapeType.hexagon;
        default: return ShapeType.circle;
    }
  }

  void _updatePhysics() {
     for (var orb in _orbs) orb.update(_shakeIntensity);
  ...
  void _resolveCollision(_Orb a, _Orb b) {
    ...
    if (dist < (rA + rB)) {
      ...
      double nx = dx / dist;
      double ny = dy / dist;
      a.alignment = Alignment(a.alignment.x - nx * overlap * 0.5, a.alignment.y - ny * overlap * 0.5);
      b.alignment = Alignment(b.alignment.x + nx * overlap * 0.5, b.alignment.y + ny * overlap * 0.5);
      Alignment temp = a.velocity; a.velocity = b.velocity; b.velocity = temp;
    }
  }

  void _initOrbs() {
    _orbs.clear();
    // Vibrant Glossy Palette
    ...
    for (int i = 0; i < 5; i++) { // Reduced to 5 orbs
  ...
  void _updateShapeType() { _currentShape = _targetShape(); }
  ...
  Alignment _randomEdgeAlignment() {
    // Generate random alignment but exclude the center box (-0.5 to 0.5)
    double x, y;
    if (_random.nextBool()) {
      // Fix X to edge, Y random
      x = _random.nextBool() ? (_random.nextDouble() * 0.4 + 0.6) : -(_random.nextDouble() * 0.4 + 0.6);
      y = _random.nextDouble() * 2.0 - 1.0;
    } else {
      // Fix Y to edge, X random
      y = _random.nextBool() ? (_random.nextDouble() * 0.4 + 0.6) : -(_random.nextDouble() * 0.4 + 0.6);
...
class _Orb {
  ...
  void update(double shakeIntensity) {
    ...
    if (nextX < -1.1 || nextX > 1.1) velocity = Alignment(-velocity.x, velocity.y);
    if (nextY < -1.1 || nextY > 1.1) velocity = Alignment(velocity.x, -velocity.y);
...
class _OrbPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // Light Grey Background for better contrast than pure white
    canvas.drawRect(Offset.zero & size, Paint()..color = const Color(0xFFF5F7FA));
    ...
    for (var orb in orbs) {
      ...
      // 1. Drop Shadow (Crucial for visibility/depth)
      canvas.drawCircle(
        center + const Offset(8, 15), 
        sRadius * 0.9, 
        Paint()..color = orb.color.withOpacity(0.3 * opacity)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 10) // Reduced blur
      );

      // 2. Main Body Gradient (More opaque core)
      ...
      // 3. Highlight Glint (Glassy effect)
      final highlightPaint = Paint()..shader = RadialGradient(
         colors: [Colors.white.withOpacity(0.9 * opacity), Colors.white.withOpacity(0.0)],
         center: Alignment.topLeft,
         radius: 0.8,
      ).createShader(Rect.fromCircle(center: center - Offset(sRadius*0.3, sRadius*0.3), radius: sRadius * 0.6));
      ...
      switch (shapeType) {
        case ShapeType.circle:
           ...
           canvas.drawCircle(Offset(-sRadius*0.3, -sRadius*0.3), sRadius*0.35, highlightPaint);
           break;
           
        case ShapeType.square:
           canvas.drawRRect(
             RRect.fromRectAndRadius(Rect.fromCenter(center: Offset.zero, width: sRadius * 1.6, height: sRadius * 1.6), Radius.circular(sRadius*0.4)),
             paint
           );
           canvas.drawCircle(Offset(-sRadius*0.4, -sRadius*0.4), sRadius*0.35, highlightPaint);
           break;
        ...
        case ShapeType.triangle:
           ...
           canvas.drawCircle(Offset(0, -sRadius*0.5), sRadius*0.3, highlightPaint);
           break;
        ...
        case ShapeType.hexagon:
           ...
           canvas.drawPath(path, paint);
           canvas.drawCircle(Offset(-sRadius*0.3, -sRadius*0.5), sRadius*0.3, highlightPaint);
           break;
      }
      canvas.restore();
    }
