...
class NutritionFirestoreService extends BaseFirestoreService {
  static final NutritionFirestoreService _instance = NutritionFirestoreService._internal();
  ...
  Future<DailyNutrition?> getDailyNutrition(String userId, DateTime date) async {
    return handleFirestoreOperation(
      () async {
        ...
        final doc = await getUserSubcollection(userId, FirestoreCollections.nutrition)
            .doc(dateKey)
            .get();
        ...
        final mealsSnapshot = await getUserSubcollection(userId, FirestoreCollections.nutrition)
            .doc(dateKey)
        ...
        return DailyNutrition(
          ...
          goal: NutritionGoal.fromJson(data[FirestoreFields.goal] as Map<String, dynamic>),
        );
      },
  ...
  Stream<DailyNutrition?> streamDailyNutrition(String userId, DateTime date) {
    final dateKey = getDateKey(date);
    final docRef = getUserSubcollection(userId, FirestoreCollections.nutrition).doc(dateKey);
  ...
  Future<void> addMeal(String userId, DateTime date, Meal meal, {NutritionGoal? currentGoals}) async {
    ensureAuthenticated();
    
    return handleFirestoreOperation(
      () async {
        final dateKey = getDateKey(date);
        final nutritionRef = getUserSubcollection(userId, FirestoreCollections.nutrition).doc(dateKey);
  ...
  Future<String> uploadMealImage(String userId, String mealId, File imageFile) async {
    ensureAuthenticated();
    // Pass userId and imageType 'meals' to LocalImageStorageService
    return _imageStorage.saveImage(imageFile, userId, 'meals');
  }

  Future<void> updateWaterIntake(String userId, DateTime date, int amount, {NutritionGoal? currentGoals}) async {
    ensureAuthenticated();
  ...
  Future<void> updateDailyGoal(String userId, DateTime date, NutritionGoal goal) async {
    ensureAuthenticated();
    ...
    return handleFirestoreOperation(
      () async {
        ...
        if (!exists) {
          ...
          // Update the goal field specifically for this day
          await nutritionRef.update({
      ...
      () async {
        ...
        if (doc.exists && doc.data() != null) {
          ...
          if (data[FirestoreFields.goal] != null) {
            return NutritionGoal.fromJson(data[FirestoreFields.goal] as Map<String, dynamic>);
          }
        }
  ...
  Future<List<DailyNutrition>> getNutritionHistory(
    ...
    return handleFirestoreOperation(
      () async {
        final snapshot = await getUserSubcollection(userId, FirestoreCollections.nutrition)
            .where(
        ...
        for (final doc in snapshot.docs) {
          ...
          final meals = mealsSnapshot.docs
              .map((mealDoc) => Meal.fromJson({...mealDoc.data() as Map<String, dynamic>, 'id': mealDoc.id}))
              .toList();
  ...
  Future<void> _initializeDailyNutrition(
    ...
    final dateKey = getDateKey(date);
    final defaultGoals = goals ?? NutritionGoal(
      dailyCalories: 2000,
      protein: 150,
      carbs: 200,
      fats: 65,
