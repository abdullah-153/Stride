...
class WeightTrackingFirestoreService extends BaseFirestoreService {
  static final WeightTrackingFirestoreService _instance = WeightTrackingFirestoreService._internal();
  ...
  Future<void> addWeightEntry(String userId, WeightEntry entry) async {
    ...
    return handleFirestoreOperation(
      () async {
        ...
        await getUserSubcollection(userId, FirestoreCollections.weightEntries)
            .doc(dateKey)
  ...
  Future<List<WeightEntry>> getWeightHistory(
    ...
    return handleFirestoreOperation(
      () async {
        Query query = getUserSubcollection(userId, FirestoreCollections.weightEntries)
  ...
  Future<WeightEntry?> getLatestWeight(String userId) async {
    return handleFirestoreOperation(
      () async {
        final snapshot = await getUserSubcollection(userId, FirestoreCollections.weightEntries)
            .orderBy(FirestoreFields.date, descending: true)
            .limit(1)
            .get();
        ...
        return WeightEntry.fromJson(snapshot.docs.first.data() as Map<String, dynamic>);
      },
