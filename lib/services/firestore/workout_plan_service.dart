...
class WorkoutPlanService extends BaseFirestoreService {
  ...
  Future<String> createWorkoutPlan(
    ...
    return handleFirestoreOperation(
      () async {
        ...
        final docRef = await getUserSubcollection(userId, 'customWorkoutPlans')
  ...
  Future<List<Map<String, dynamic>>> getUserWorkoutPlans(String userId) async {
    return handleFirestoreOperation(
      () async {
        final snapshot = await getUserSubcollection(userId, 'customWorkoutPlans')
  ...
  Future<Map<String, dynamic>?> getWorkoutPlan(String userId, String planId) async {
    return handleFirestoreOperation(
      () async {
        final doc = await getUserSubcollection(userId, 'customWorkoutPlans')
            .doc(planId)
            .get();
  ...
  Future<void> updateWorkoutPlan(
    ...
    return handleFirestoreOperation(
      () async {
        ...
        await getUserSubcollection(userId, 'customWorkoutPlans')
            .doc(planId)
  ...
  Future<void> setActiveWorkoutPlan(
    ...
    return handleFirestoreOperation(
      () async {
        await getUserSubcollection(userId, 'activeWorkoutPlan')
            .doc('data')
            .set({
          'planId': planId,
  ...
  Future<Map<String, dynamic>?> getActiveWorkoutPlan(String userId) async {
    return handleFirestoreOperation(
      () async {
        final doc = await getUserSubcollection(userId, 'activeWorkoutPlan')
            .doc('data')
            .get();
  ...
  /// Mark workout as completed in active plan
  ...
  /// Progress to next day/week in plan
  Future<void> progressPlan(String userId, int currentWeek, int currentDay) async {
    ensureAuthenticated();
