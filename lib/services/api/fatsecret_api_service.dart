...
class FatSecretApiService {
  ...
  static const String _autocompleteUrl = 'https://platform.fatsecret.com/rest/food/autocomplete/v2';
  ...
  Future<String> _getAccessToken() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _tokenExpiry = DateTime.now().add(Duration(seconds: expiresIn - 60)); // Buffer
        return _accessToken!;
      } else {
  ...
  Future<List<String>> getSuggestions(String query) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$_autocompleteUrl?expression=${Uri.encodeComponent(query)}&format=json&max_results=10'),
        headers: {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        // Response format: { "suggestions": { "suggestion": [ "term1", "term2" ] } }
        // OR if single result: { "suggestions": { "suggestion": "term1" } }
        // OR if no result: { "suggestions": null } or empty?

        if (data['suggestions'] != null && data['suggestions']['suggestion'] != null) {
        ...
        print('FatSecret Autocomplete Error: ${response.statusCode} ${response.body}');
        return [];
      }
    } catch (e) {
  ...
  static const String _searchUrl = 'https://platform.fatsecret.com/rest/foods/search/v1';
  static const String _foodGetUrl = 'https://platform.fatsecret.com/rest/food/v2';

  /// Search for foods (full search with parsing)
  Future<List<Map<String, dynamic>>> searchFoods(String query) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$_searchUrl?search_expression=${Uri.encodeComponent(query)}&format=json&max_results=20'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
  ...
  Future<Map<String, dynamic>?> getFoodById(String foodId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('FatSecret Get Food Error: ${response.statusCode} ${response.body}');
        return null;
      }
    } catch (e) {
      print('FatSecret Exception: $e');
  ...
  /// Parse search result (Food Summary)
  Map<String, dynamic> _parseFoodSummary(Map<String, dynamic> food) {
    // "Per 100g - Calories: 100kcal | Fat: 2.00g | Carbs: 10.00g | Protein: 5.00g"
    ...
    return {
      'id': food['food_id'],
      'name': food['food_name'],
      // Standardize fields for Meal model
  ...
  /// Parse "Per 100g - Calories: 100kcal | Fat: 2.00g | Carbs: 10.00g | Protein: 5.00g"
  ...
  Map<String, dynamic> _parseFoodDetails(Map<String, dynamic> food) {
    ...
    return {
      ...
      'calories': double.tryParse(defaultServing['calories'] ?? '0')?.toInt() ?? 0,
      ...
      'servingSize': double.tryParse(defaultServing['metric_serving_amount'] ?? '100') ?? 100.0,
